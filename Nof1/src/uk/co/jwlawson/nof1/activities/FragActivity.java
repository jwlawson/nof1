...
public class FragActivity extends SherlockFragmentActivity {
	...
	private class FragLoader extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			// Add some questions
			String[] qus = new String[] { "On average, in comparison to your usual episodes, how long were the attacks?",
			...
			for (int i = 0; i < qus.length; i++) {
				RadioFragment frag = RadioFragment.newInstance(qus[i], min[i], max[i]);
				switch (column) {
				case 0:
			...
			CheckFragment chkFrag1 = CheckFragment.newInstance(qus2[0], false);
			CheckFragment chkFrag2 = CheckFragment.newInstance(qus[1], true);
			...
			ft0.commit();
			if (BuildConfig.DEBUG) Log.d(TAG, "Radios created");

			// Add number fragments
			NumberFragment num1 = NumberFragment.newInstance("How many times have you fallen over today?");
			NumberFragment num2 = NumberFragment.newInstance("On average how many metres can you walk without getting out of breath?");
